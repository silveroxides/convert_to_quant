...
class QuantizationErrorMeasurer:
    ...
    def __init__(self, device: str = "cuda" if torch.cuda.is_available() else "cpu", low_memory: bool = False):
        self.device = device
    ...
    def dequantize_fp8(self, quantized: torch.Tensor, scale: torch.Tensor, original_dtype: torch.dtype) -> torch.Tensor:
    ...
    def get_scale_from_tensors(self, tensors: Dict[str, torch.Tensor], layer_name: str) -> Optional[torch.Tensor]:
        """Extract scale tensor for a quantized layer."""
        # Try common scale tensor naming conventions
        scale_names = [f"{layer_name}.weight_scale", f"{layer_name}.scale", f"{layer_name}_scale"]
    ...
    def get_comfy_quant_config(self, tensors: Dict[str, torch.Tensor], layer_name: str) -> Optional[Dict]:
    ...
    def compare_models(self, original_tensors: Dict[str, torch.Tensor], quantized_tensors: Dict[str, torch.Tensor], original_metadata: Optional[Dict] = None, quantized_metadata: Optional[Dict] = None, layer_filter: Optional[List[str]] = None) -> bool:
        ...
        for layer_name in matched_pairs:
            ...
            if original_tensor.shape != quantized_tensor.shape:
                warning(f"  Shape mismatch for {layer_name}: {original_tensor.shape} vs {quantized_tensor.shape}")
                continue
    ...
    def compute_aggregate_metrics(self) -> Optional[AggregateErrorMetrics]:
        ...
        if total_error_norm > 1e-10:
            overall_snr_db = 20.0 * torch.log10(torch.tensor(total_original_norm / total_error_norm)).item()
        else:
    ...
    def save_report(self, filepath: str, aggregate: AggregateErrorMetrics):
        ...
        report = {"aggregate": asdict(aggregate), "layers": [asdict(m) for m in self.layer_metrics]}
