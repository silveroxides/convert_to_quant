...
def create_synthetic_fp8_quantized_model(bf16_tensors: dict, output_path: str, per_tensor_scale: bool = True):
    ...
    for key, original in bf16_tensors.items():
        ...
        comfy_quant_config = {"format": "float8_e4m3fn"}
        tensors[f"{layer_name}.comfy_quant"] = create_comfy_quant_tensor("float8_e4m3fn")

        # Add to quantization metadata
        quant_metadata_layers[f"{layer_name}"] = {"format": "float8_e4m3fn", "dtype": "float8_e4m3fn"}

        verbose(f"  Quantized {key}: scale={scale.item():.6e}")

    # Create quantization metadata for header
    quant_metadata = {"format_version": "1.0", "layers": quant_metadata_layers}

    # Save model with metadata
    os.makedirs(os.path.dirname(output_path) if os.path.dirname(output_path) else ".", exist_ok=True)
    save_file(tensors, output_path, metadata={"_quantization_metadata": json.dumps(quant_metadata)})
...
def main():
    ...
    try:
        ...
        # Compare
        measurer.compare_models(original_tensors, quantized_tensors, original_metadata, quantized_metadata)
